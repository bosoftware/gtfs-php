...
do
    ...
    stops.txt)
	    ...
	    tail -n +2 $file | sort -k6 -g -t, | grep -n '^' - | sed -e '/^$/d' | sed -e 's/,\ */,/g' | sed -e 's/_merged_[0-9]*//g' | sed -e 's/\([0-9]\):/\1,/g' > $tmpfile
	    (
    ...
    stop_times.txt)
	    ...
	    tail -n +2 $file | sed -e '/^$/d' | sed -e 's/,\ */,/g' | sed -e '/^\s*$/d' | sed -e 's/_merged_[0-9]*//g' | sed -e 's/\,\ \([0-9]\):\([0-9][0-9]\):\([0-9][0-9]\)/\,0\1\2\3/g' | sed -e 's/\,\([0-9]\):\([0-9][0-9]\):\([0-9][0-9]\)/\,0\1\2\3/g' | sed -e 's/://g' > $tmpfile
    ...
    routes.txt|trips.txt)
	    ...
	    #tail -n +2 "$file" | tr '\r' '\n' > $tmpfile
	    tail -n +2 "$file" | tr -d '\015' | sed -e '/^$/d' | sed -e 's/_merged_[0-9]*//g' | sed -e 's/,\ */,/g' > $tmpfile
	    (
		echo "create table $table($columns);"
		echo ".separator ,"
		echo ".import $tmpfile $table"
